...
module.exports = (io) => {
    ...
    let activity_mod = D.model('activity_yhb')
        ...
        socket.on('init', async (d)=> {
            if (d.id) {
                ...
                if(cache[socket.roomId].activity){
                    sc_screen.in(socket.roomId).emit('activity', {
                        activity: cache[socket.roomId].activity
                    })
                }else{
                    let activity_data = await activity_mod.find({aid:d.id}).toPromise()
                    //console.log('screeen',activity_data)
                    activity_data = activity_data[0]
                    if(activity_data){
                        cache[socket.roomId].activity = activity_data
                        sc_screen.in(socket.roomId).emit('activity', {
                            activity: cache[socket.roomId].activity
                        })
                    }
            ...
            if (d.id) {
                ...
                if (d.member && d.member.token && d.host) {
                    ...
                    if (!cache[socket.roomId].activity) {
                        ...
                        let activity = await activity_mod.find({aid:d.id}).toPromise()
                        activity = activity[0]
                        if(!activity) {
                            let ylpHost = ylpUrl(d.host)
                            activity = await getData(ylpHost + d.id, d.member.token)
                            //console.log('client',activity)
                            if (activity && activity.code == 0) {
                                activity = activity.data
                                let activity_data = activity
                                activity_data.aid = d.id
                                await activity_mod.create(activity_data).toPromise()                            }
                        }
                ...
                if(cache[socket.roomId].activity){
                    //////////////////////////////
                    sc_client.in(socket.roomId).emit('activity', {
                        activity: cache[socket.roomId].activity
                    })
